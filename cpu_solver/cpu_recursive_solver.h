...
int cpuPreprocessSudoku(Sudoku* sudoku, const int sudokuNo) {
    for (int i = 0; i < SUDOKU_DIMENSION_SIZE; i++) {
        for (int j = 0; j < SUDOKU_DIMENSION_SIZE; j++) {
            const uint32_t digit = getDigitAt(sudoku, sudokuNo, i, j);

            if (digit != 0) {
                const uint16_t possible = getPossibleDigitsAt(sudoku, sudokuNo, i, j);
                if (possible >> (digit - 1) & ONE_BIT_MASK == 0)
                    return -1;

                updateUsedDigitsAt(sudoku, sudokuNo, i, j, digit);
            }
        }
    }
    ...
    for (int i = 0; i < SUDOKU_DIMENSION_SIZE; i++)
        for (int j = 0; j < SUDOKU_DIMENSION_SIZE; j++) {
            ...
            int digits = getPossibleDigitsAt(sudoku, sudokuNo, i, j) & NINE_BIT_MASK;

            if (__builtin_popcount(digits) == 1) {
                setDigitAndUpdateUsedDigits(sudoku, sudokuNo, i, j, __builtin_ffs(digits));
                restart = 1;
            }
        }

    if (restart)
        return cpuPreprocessSudoku(sudoku, sudokuNo);
...
int validateSudokuSolution(const Sudoku* sudoku, const int sudokuNo) {
    Sudoku validator = {};
    ...
    for (int i = 0; i < SUDOKU_DIMENSION_SIZE; i++)
        for (int j = 0; j < SUDOKU_DIMENSION_SIZE; j++) {
            ...
            if (checkIfDigitIsPossible(&validator, 0, i, j, digit) <= 0)
                return -1;

            setDigitAndUpdateUsedDigits(&validator, 0, i, j, digit);
        }
